...
const modalTitle = computed(() => (editandoId.value ? 'Editar servicio' : 'Nuevo servicio'));
const saveLabel = computed(() => (editandoId.value ? 'Guardar cambios' : 'Crear servicio'));
...
const guardar = async () => {
  ...
  if (result.ok) {
    toast.success(editandoId.value ? 'Servicio actualizado' : 'Servicio creado');
    cerrarModal();
...
const desactivar = async (item) => {
  if (!confirm(`¿Desactivar el servicio "${item.nombre}"?`)) return;
...
<template>
  <section>
    <PageToolbar title="Servicios" subtitle="Catálogo de servicios ejecutables.">
      <template #actions>
